...
{if !$page || $page == 0}
	...
	{literal}
	<!-- script type="text/javascript" src="http://www.maryjane.ru/js/tiny_mce/tiny_mce.js"></script-->
	<script type="text/javascript">
	...
	tinyMCE.init({
		...
		content_css : "http://www.maryjane.ru/css/tinymce_css.css",
		setup : function(ed) {
...
<div class="commentColumn">
	...
	<div id="comment{$c.comment_id}" class="comment clr {$c.class} {$c.author_comment}">
		...
		<tr valign="top">
			<td width="60" valign="top"><!--noindex--><a href="http://www.maryjane.ru/profile/{$c.user_id}/" title="{$c.user_login}" rel="nofollow">{$c.user_avatar}</a><!--/noindex--></td>
			<td style="overflow:hidden">
				<h6 style="margin-top:0px;"><a href="http://www.maryjane.ru/profile/{$c.user_id}/" title="{$c.user_login}" rel="nofollow">{$c.user_login}</a> {$c.user_designer_level}&nbsp;
				<!--noindex--><font class="em">{$c.date}</font>
				{* &nbsp;&nbsp;&nbsp;<a href="#" title="Цитировать" class="citate" onclick="return citateComment('{$c.comment_id}', '{$type}');" rel="nofollow"><img src="/images/icons/citata.gif" alt="Цитировать" class="citate" border="0" /></a> *}<!--/noindex-->
				</h6>
		...
		<div style="font-size:10px;" class="actions">
			...
			<small class="citate"><!--noindex--><a name="comment{$c.comment_id}" href="#comment{$c.comment_id}"  onclick="return citate('{$c.comment_id}', '{$type}');" rel="nofollow">{if $module=='catalog.v2'}<br replaces="ответить"/>{else}ответить{/if}</a><!--/noindex--></small>
...
{*if $rest > 0}
<br clear="all" />
<p><a href="/ajax/getTab/?good_id={$good.good_id}&tab=comments&page={$page + 1}" class="load-comment-ajax">показать ещё {$rest}</a></p>
{/if*}
