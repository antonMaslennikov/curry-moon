...
{foreach from=$goods item="g" key="k"}	
	<li class="m12 {if $g.hidden && $g.user_id != $USER->id}hidden{/if}" page="{$page}">
		<a rel="nofollow" class="item" title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" href="{if $g.link}{$g.link}{else}/catalog/{$g.user_login}/{$g.good_id}/category,phones/#{$style_slug}{/if}">
			<span class="list-img-wrap" style="text-align:center"><img title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" alt="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if} " style="background-color: #{$g.bg};" src="{$g.picture_path}" width="210" /></span>
		</a>
		
		<a rel="nofollow" href="{$g.zoomlink}" class="zoom"></a>
		...
		<div class="item">
			...
			<div class="price-wrap gadget">
				<span>цена:</span>
				<span class="price {if $stock.status == 'preorder'}predzalaz{/if} {if $g.price_old > $g.price}gold{/if}">{if $stock.status == 'preorder'}Предзаказ, {/if}{if $g.price_old > $g.price}<strike>{$g.price_old}</strike>{/if} {$g.price}&nbsp;{$L.CURRENT_CURRENCY}</span>
