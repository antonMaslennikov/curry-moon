...
{foreach from=$goods item="g" key="k"}	
		<li class="m12  {if $g.hidden && $g.user_id != $USER->id}hidden{/if} {$style_slug}">
			<a rel="nofollow" class="item" title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" href="{if $g.link}{$g.link}{else}/catalog/{$g.user_login}/{$g.good_id}/category,phones/#{$style_slug}-bumper{/if}">
				<span class="list-img-wrap">
					...
					<span title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" alt="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if} " />
				</span>
			</a>
			...
			<div class="item">
				<a {if $k > 2}rel="nofollow"{/if}  class="title" title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" href="{if $g.link}{$g.link}{else}/catalog/{$g.user_login}/{$g.good_id}/category,phones/#{$style_slug}-bumper{/if}">{$g.good_name}</a>
				...
				<br/>
				<!--зелёный блок-->
				<!--noindex--><span class="price">800&nbsp;руб.</span><!--/noindex-->
			</div>	
...
{if $rest > 0}
<div style="position:relative;float:left;width:100%;left:-25px;" parentlist="true">
