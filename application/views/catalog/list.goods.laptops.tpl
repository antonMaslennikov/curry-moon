...
{foreach from=$goods item="g" key="k"}
	<li class="m12 {if $g.hidden && $g.user_id != $USER->id}hidden{/if}" page="{$page}">
		<a {if $k > 2}rel="nofollow"{/if} class="item" title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" href="{if $g.link}{$g.link}{else}/catalog/{$g.user_login}/{$g.good_id}/category,laptops/#{$style_slug}{/if}">
		...
		<a class="btn_editing" href="/senddrawing.design/{$g.good_id}/" rel="nofollow"></a>
			{if $USER->user_id == 27278 || $USER->user_id == 6199 || $USER->user_id == 105091}
				<a class="btn_editing krest"title="" href="#" onclick="tb_show('', '/senddrawing.design/{$g.good_id}/stickermize/{$category}/?width=930&height=620');return false;"></a>
			{/if}
		{/if}			
		<div class="vote{if $g.liked} select{/if} {if $g.place > 0} heart_red {/if}" {* like-name="laptops" *} _id="{$g.good_id}"><span>{if $g.good_status != "voting"}{$g.likes}{else}-{/if}</span></div>
		...
		<div class="item">
			...
			<div class="price-wrap gadget">
				<span>цена:</span>
				<span class="price {if $g.price_old > $g.price}gold{/if}">{if $g.price_old > $g.price}<strike>{$g.price_old}</strike>{/if} {$g.price}&nbsp;{$L.CURRENT_CURRENCY}</span>
