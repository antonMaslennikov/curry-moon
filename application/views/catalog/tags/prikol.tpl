...
.sidebar_filter .b-radio-manwomen {margin-top:-28px!important;}
</style>
<!--[if IE]>
	<style>
...
<div class="b-catalog_v2 moveSidebarToLeft {if $filters.category}b-catalog_v3{else}{/if}">
...
{/if}
	...
	{*<a href="/8march/new/" title="8 марта" rel="nofollow" id="top2014"></a>
	<a href="/23february/new/" title="23 февраля" rel="nofollow" id="zvezda" ></a>*}
	...
	<div class="NLtags">
			...
			$(document).ready(function() { 	
				$('.pageTitle select.tags').change(function(){
					if ($(this).val().length > 0)
						location.href ='/tag/'+ $(this).val()+($(this).children('option:selected').attr('_category').length > 0?'/category,'+ $(this).children('option:selected').attr('_category'):'')+'/'; 
				});
			});
			</script>
			<style>
		...
		{/literal}
			<div class="b-filter_view">
				...
				<a href="{$mlink}ajax/" rel="nofollow" class="view-other active" title=""></a>
			</div>		
...
<script>
	var urlViews = ["/{$module}/|%sort%|/ajax/{if $filters.category}category,{$filters.category}{if $filters.size && !$fsize_not_selected};size,{$filters.size}{/if}{if $filters.color && !$fcolor_not_selected};color,{$filters.color}{/if}/{/if}{if $filters.sex == 'female'}female/{/if}", "/catalog/|%sort%|/ajax"];
</script>
...
<div class="catalog_goods_list {if !$filters.category}neformat{else}list_trjapki{/if}">
