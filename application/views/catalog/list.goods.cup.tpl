    ...
    <li class="m12 {if ($USER->id == 27278 || $USER->id == 6199 || $USER->id == 105091) && $g.hidden}hidden{/if}" {if $USER->id == 27278 || $USER->id == 6199 || $USER->id == 105091}sexLeft="{$g.good_sex}"{/if} {if $g.good_status != 'archived'}page="{$page}"{/if}>
		...
		<div class="item">
			...
			<div class="price-wrap">
				<span>цена:</span>
				<span class="price {if $g.price_old > $g.price}gold{/if}">{if $g.price}{if $g.price_old > $g.price}<strike>{$g.price_old}</strike>{/if} {$g.price}&nbsp;{$L.CURRENT_CURRENCY}{else}<strike>1090</strike> 790&nbsp;руб.{/if}</span>
				<div class="one_click_order_block">
					<a href="#" onclick="return quickBuy('{$g.good_id}', '{$stock.good_stock_id}');" _href="/order.v3/" rel="nofollow">Купить</a>
				</div>
			</div>
