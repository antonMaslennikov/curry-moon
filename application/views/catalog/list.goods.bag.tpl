...
{foreach from=$goods item="g" key="k"}	
	<li class="m12 {if $g.hidden && $g.user_id != $USER->id}hidden{/if}">
		...
		{if $g.price_old > $g.price}
			<div class="red-sale-stiker"><strike>{$g.price_old} {$L.CURRENT_CURRENCY}</strike><span>{$g.price} {$L.CURRENT_CURRENCY}</span></div>
		{/if}
		...
		<div class="item">
			<a {if $k > 2}rel="nofollow"{/if} class="title" title="{$g.good_name} - {if $PAGE->utitle}{$PAGE->utitle}{else}{$PAGE->title}{/if}" href="{if $g.link}{$g.link}{else}/catalog/{$g.user_login}/{$g.good_id}/category,phones/#{$style_slug}{/if}">{$g.good_name}</a>			
			...
			<br/>
			<!--зелёный блок-->
			<!--noindex--><span class="price {if $stock.status == 'preorder'}predzalaz{/if} {if $g.price_old > $g.price}gold{/if}">{if $stock.status == 'preorder'}Предзаказ, {/if}{if $g.price_old > $g.price}<strike>{$g.price_old}</strike>{/if} {$g.price}&nbsp;{$L.CURRENT_CURRENCY}</span><!--/noindex-->
			{*<!--noindex--><span class="price {if $g.price_old > $g.price}gold{/if}">{if $g.price_old > $g.price}<strike>{$g.price_old}</strike>{/if} {$g.price}&nbsp;{$L.CURRENT_CURRENCY}</span><!--/noindex-->*}
		</div>
...
<a class="show-more-link" id="show-more-link"  href="{$link}/page/{$page + 1}/ajax/{if $SEARCH}?q={$SEARCH}{/if}" rel="nofollow" style="height:200px;">Показать ещё {*$rest*}</a><div class="allpager" title="Включить автолистание"></div>
</div>
{/if}